...
class Graph extends GraphEvent {
  ...
  addNode(options) {
    const node = options.constructor === GraphNodes
      ? options
  ...
  addLink(options) {
    const newLink = options.constructor === GraphLinks
      ? options
  ...
  removeNode(node) {
    ...
    this.linkList.filter((link) => link.start === node || link.end === node).forEach((link) => {
