  ...
  <div class="super-flow" ref="flow-canvas" @contextmenu.prevent.stop="contextmenu">
    <div class="dragBg" ref="dragBg">
      <graph-line
        ...
        v-model:mouseonLink="graph.mouseonLink"
      ...
      <graph-node
        ...
        :node="node"
        :graph="graph"
        v-model:mouseonNode="graph.mouseonNode"
        :origin="graph.origin"
        :key="node.key"
...
import GraphLine from './link.vue'
...
export default {
  ...
  methods: {
    ...
    showContextMenu(position, list, source) {
      ...
      this.menuConf.origin = source.origin
    ...
    // 改变起点,连线有动画
    moveTemEdge(evt) {
      this.temEdgeConf.link.movePosition = NewCoordinate(getOffset(evt, this.$el))
    },
    ...
    changeMovePalette(val) {
      this.MovePalette = val
    },
  },
  // FIXME 读懂watch
  watch: {
...
.dragBg {
  ...
  background-image: linear-gradient(90deg, #ebebeb 5%, transparent 0), linear-gradient(#ebebeb 5%, transparent 0);
