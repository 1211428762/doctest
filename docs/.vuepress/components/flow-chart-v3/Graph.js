...
import GraphNodes from './GraphNode'
import GraphLinks from './GraphLink'
...
class Graph extends GraphEvent {
  ...
  createNode(options) {
    return new GraphNodes(options, this)
  }

  createLink(options) {
    return new GraphLinks(options, this)
  }

  addNode(options) {
    const node = options.constructor === GraphNodes ? options : this.createNode(options)
  ...
  addLink(options) {
    const newLink = options.constructor === GraphLinks ? options : this.createLink(options)

    const currentLink = this.linkList.find((item) => item.start === newLink.start && item.end === newLink.end)
  ...
  removeNode(node) {
    ...
    this.linkList
      .filter((link) => link.start === node || link.end === node)
      .forEach((link) => {
