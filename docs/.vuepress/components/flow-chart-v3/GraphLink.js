...
function pathDirection(vertical, horizontal, directionParam) {
  if (
    vector(horizontal)
      .parallel(directionParam)
      .end
  ) {
    if (
      vector(horizontal)
        .dotProduct(directionParam)
        .end > 0
    ) {
      return horizontal;
    }
...
export default class GraphLink {
  ...
  coordinateList(turnRatio = 0.5) {
    ...
    const startDirection = pathDirection(
      pathVerticalVec,
      pathHorizontalVec,
      entryDirection,
    );
    const endDirection = pathDirection(
      pathVerticalVec,
      pathHorizontalVec,
      exitDirection,
    );
